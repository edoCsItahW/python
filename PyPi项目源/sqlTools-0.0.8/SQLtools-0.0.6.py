...
from typing import Callable, Literal, final, Final
...
class baseSQL:
    ...
    def __init__(self, user: str, password: str, database: str = None, *, host: str = 'localhost', tableName: str = None):
        self._user = user
    ...
    @errorf()
    def selectColumn(self, tbName: str | tuple[str] = None, columns: tuple[str, EllipsisType] = None, *, condition: Literal["where ... and/or ... (like %str )"] | str = None, show: bool = True) -> list:
        """
        返回或展示数据表中的某些列的内容.
        ...
        :type columns: tuple
        :param condition: 符合mysql语法的条件,如:`WHERE COLUMN1 > 0`
        :type condition: str
        :param show: 是否打印以展示.
        :type show: bool
    ...
    @errorf()
    def createTable(self, tableName: str, id_AUTO: bool | str = True, primarykey: str = None, show: bool = True, **kwargs: tuple[tuple[str,],] | tuple[tuple[str, int],] | tuple[
                        tuple[str,] | tuple[str, int], bool, str | bool, bool] | tuple[
                                  tuple[str,] | tuple[str, int], bool, str | bool, bool, str | None]) -> None:
        r"""
        创建数据表.
        ...
        :param show: 是否打印以展示.
        :type show: bool
        :param kwargs: 指定表中的列名和其定义,且参数应形如'列名=定义元组',其中元组应形如:
            1.((类型, 长度[可选可空]), )
    ...
    def insert(self, tbName: str = None, show: bool = True, **kwargs) -> None:
        ...
        :param show: 是否打印以展示.
        :type show: bool
        :param kwargs: 添加的数据,应形如'列名=对应数据'
        :type kwargs: ...
        :return: 返回什么.
        :retype: 返回值的类型
        """
    ...
    def update(self, tbName: str = None, condition: Literal["where ... in ...", "where ... and/or ..."] | str = None,
        ...
        :param check: 是否进行安全检查.
        :type check: bool
        :param kwargs: 修改的数据,应形如'列名=对应数据',(仅输入需要修改的列即可.)
        :type kwargs: ...
        :return: 操作执行函数不做返回.
        :retype: None
        """
    ...
    def column_add(self, tbName: str = None, columnName: str = None, dataType: str = None, *, First: bool = False,
        ...
        :param dataType: 数据类型.
        :type dataType: str
        :param First: 插入到所有列之前.
        :type First: bool
        :param After: 插入到某一列之后,应输入目标的列名,且不可于First同时使用.
        :type After: str
        :param notNull: 是否可为空.
        :type notNull: bool
        :param show: 是否打印以展示.
    ...
    @errorf()
    def to_csv(self, csv_path: str, tbName: str, *, condition: Literal["where ... = ...", "where ... and/or ..."] | str = None) -> None:
        """
        将数据库导出为csv文件.
        ...
        velous = self.showTableContent(tbName, show=False) if condition is None else self.selectColumn(columns=("*",), condition=condition, show=False)
    ...
    def checkId(self, tbName: str = None, *, firstNum: int = 1):
        if tbName is None:
            tbName = self.tbName
    ...
    @errorf()
    def randomChoice(self, tbName: str = None, *, condition: Literal["where ... in ...", "where ... and/or ..."] | str = None, limit: int = 1):

        if tbName is None:
            tbName = self.tbName

        self._cursor.execute(f"SELECT * FROM {tbName}{'' if condition is None else ' ' + condition} ORDER BY RAND() LIMIT {limit};")
        self._connect.commit()
