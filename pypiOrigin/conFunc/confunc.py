...
def sequence(index: int | tuple | Container, *, start: int = 0) -> list:
    ...
    :retype: 返回值的类型
    """
    return list(range(start, len(index) + start))


@sequence.register(int)
def _(index: int, *, start: int = 0) -> list: return list(range(start, index + start))


@sequence.register(tuple)
def _(index: tuple, *, start: int = 0) -> list: return list(range(index[0], index[1]))
