...
class MCRcon(object):
    ...
    def _send(self, out_type, out_data):
        ...
        out_payload = (
            pack("<ii", 0, out_type) + out_data.encode("utf8") + b"\x00\x00"
        )
    ...
    def command(self, command):
        ...
        sleep(0.003)  # MC-72390 workaround
        return result
...
def mcrcon_cli():
    try:
        ...
        if not args.password and not environ.get("RCON_PASSWORD"):
            password = getpass("Password: ")
        elif environ.get("RCON_PASSWORD"):
            password = environ.get("RCON_PASSWORD")
        else:
