...
class baseSQL:
    ...
    def __init__(self, user: str, password: str, database: str = None, *,
                 host: str = 'localhost', tableName: str = None):
        self._user = user
    ...
    @errorf()
    def selectColumn(self, tbName: str | tuple[str] = None, columns: tuple[str, ...] = None, *,
    ...
    @errorf()
    def createTable(self, tableName: str, id_AUTO: bool | str = True, primarykey: str = None, show: bool = True,
                    **kwargs: tuple[tuple[str,],] | tuple[tuple[str, int],] | tuple[
        ...
        :param kwargs: 指定表中的列名和其定义,且参数应形如'列名=定义元组',其中元组应形如:
            1.((列名, 长度[可选可空]), )
            2.((列名, 长度[可选可空]), 是否可为空, 默认值, 自增,)
            3.((列名, 长度[可选可空]), 是否可为空, 默认值, 自增, 其它约束)
        :return: 操作执行函数不做返回.
        :retype: None
        """
    ...
    @errorf()
    def executeOhter(self, command: str = ..., *, _input: bool = False, allowprint: bool = False,
