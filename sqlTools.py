...
try:
    ...
    try:
        ...
        raise e from RuntimeError(
            "没有安装ANSIdefine模块,使用`pip install -i https://test.pypi.org/simple ANSI-Define`安装。")
...
class baseSQL:
    ...
    @property
    def tbName(self): return self._tbName

    @tbName.setter
    def tbName(self, value):
        if value in self.allTable():
            self._tbName = value

        else:
            raise ValueError(  # 表不存在错误
                f"table name '{value}' does not exist!")
    ...
    def TABLE(self):
        ...
        :retype: list
        """
        return self.allTable(show=True)

    def allTable(self, *, show: bool = False) -> list[str]:
        """
        展示并返回当前数据库中的所有表.(与self.TABLE不同的是,self.TABLE在获取所有表的同时必须打印sql命令执行过程)
    ...
    def createTable(self, tableName: str, id_AUTO: bool | str = True, primarykey: str = None, show: bool = True,
                    **kwargs: con1 | con2 | con3 | con4) -> None:
        r"""
        创建数据表.
    ...
    @errorf()
    def randomChoice(self, tbName: str = None, *, condition: Literal["where ... in ...", "where ... and/or ..."] | str = None, limit: int = 1) -> list:
        # asd
        """
        随机选择数据表中的`limit`条数据
    ...
    def sqlToJson(self):
        ...
        :rtype:
        """
        return [{k: v for k, v in zip(self.COLUMN, t)} for t in self.showTableContent(show=False)]


if __name__ == '__main__':
    sql = baseSQL("root", "135246qq", "quesinfo", tableName="politics")
    # sql.createTable("lookup", False, "name", True, name=(("v", 16), True, False, False), chName=(("v", 16), True, False, False), type=(("v", 16), True, False, False))
    # sql.executeOhter(_input=True, allowprint=True, circulate=True)
    # sql.tbName = "lookup"
    # sql.insert(None, name="multChoice", chName="多选", type="option")
    # sql.column_add(None, "note", "varchar(128)", After="other", notNull=False)
    # sql.column_default(None, "note", "''")
    # sql.update(None, "where id = 1", note="")
    # sql.showTableContent()
    # sql.selectColumn(None, ("name", ), condition="where type = 'option'")
    print({"sChoice": [{k: v for k, v in zip(sql.COLUMN, t)} for t in sql.selectColumn(None, ("*", ), condition="where type = 'sChoice'", show=False)]})
