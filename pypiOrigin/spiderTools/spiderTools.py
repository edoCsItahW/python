...
class checktag:
    ...
    def findAllInSoup(soup: BeautifulSoup, tag: str, *,
                      ...
                      wordinkey: Annotated[tuple[str, str], "word in dict[key]"] = None, keynotindict: str = None, wordnotinkey: tuple[str, str] = None, getkey: str = None, test: bool = False):
        ...
        Wordinkey = lambda elem: (wordinkey[1] in elem.attrs and wordinkey[0] in elem.attrs[wordinkey[1]]) if wordinkey else True
        Keynotindict = lambda elem: (keyindict not in elem.attrs) if keyindict else True
        Wordnotinkey = lambda elem: (wordinkey[1] in elem.attrs and wordinkey[0] not in elem.attrs[wordinkey[1]]) if wordnotinkey else True
        ...
        for element in soup.find("body").find_all(tag):
            if Classis(element) and Idis(element) and Keyindict(element) and Wordinkey(element) and Wordnotinkey(element):
